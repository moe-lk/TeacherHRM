...
if(isset($_POST["FrmSrch"]) || $fm==''){
	...
	$sqlSrch="SELECT * FROM CD_ServiceRecType where Description!=''";  
...
if(isset($_POST["FrmSubmit"])){	
	...
	$RecType=trim($_REQUEST['RecType']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
    if ($Description == "") {
        $msg.= "Please enter the Service Record Type Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_ServiceRecType where DutyCode='$DutyCode'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Service Record Code.<br>";
				}else{
					 $queryMainSave = "INSERT INTO CD_ServiceRecType
					   (DutyCode,Description,RecType,RecordLog)
				 VALUES
					   ('$DutyCode','$Description','$RecType','$RecordLog')";
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_ServiceRecType SET DutyCode='$DutyCode',Description='$Description',RecType='$RecType', RecordLog='$RecordLog' WHERE DutyCode='$DutyCode'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Record Type Name :</td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="DutyCode" type="text" class="input3" id="DutyCode" value="<?php echo $DutyCode ?>" size="5" maxlength="5" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      <td>Record Type Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                    ...
                    <tr>
                      <td>Record Code</td>
                      <td>:</td>
                      <td><input name="RecType" type="text" class="input2" id="RecType" value="<?php echo $RecType ?>" size="2" maxlength="2"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
