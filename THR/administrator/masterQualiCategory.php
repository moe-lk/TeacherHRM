...
if(isset($_POST["FrmSrch"]) || $fm==''){
	...
	$sqlSrch="SELECT * FROM CD_QualificationCategory where Description!=''";  
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_QualificationCategory where Code='$id'";  
...
if(isset($_POST["FrmSubmit"])){	
	...
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_QualificationCategory where Code='$Code'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Qualification Code.<br>";
				}else{
					$queryMainSave = "INSERT INTO CD_QualificationCategory
				   (Code,Description,RecordLog)
				 VALUES
				   ('$Code','$Description','$RecordLog')";//,'$DutyCode'
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_QualificationCategory SET Code='$Code',Description='$Description', RecordLog='$RecordLog' WHERE Code='$Code'";//DutyCode='$DutyCode',
