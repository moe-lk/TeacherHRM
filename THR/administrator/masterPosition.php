...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$PositionNameSrc=$_REQUEST['PositionNameSrc'];
	$sqlSrch="SELECT * FROM CD_Positions where PositionName!=''";  
	if($PositionNameSrc)$sqlSrch.=" and PositionName like '%$PositionNameSrc%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_Positions where Code='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$Code = trim($rowE['Code']);
	$PositionName = trim($rowE['PositionName']);
	$CPCode = trim($rowE['CPCode']);
...
if(isset($_POST["FrmSubmit"])){	
	...
	$CPCode=trim($_REQUEST['CPCode']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
    if ($PositionName == "") {
        $msg.= "Please enter the Position Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
			$countSql="SELECT * FROM CD_Positions where Code='$Code'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Position Code.<br>";
				}else{
				$queryMainSave = "INSERT INTO CD_Positions
				   (Code,PositionName,CPCode,RecordLog)
			 VALUES
				   ('$Code','$PositionName','$CPCode','$RecordLog')";
				$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Positions SET Code='$Code',PositionName='$PositionName',RecordLog='$RecordLog' WHERE Code='$Code'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Position Name :</td>
                      <td width="36%"><input name="PositionNameSrc" type="text" class="input2_n" id="PositionNameSrc" value="<?php echo $PositionNameSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td>Position Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="PositionName" type="text" class="input2" id="PositionName" value="<?php echo $PositionName ?>"/></td>
                    </tr>
                     <tr>
                      <td>CP Code</td>
                      <td>:</td>
                      <td><input name="CPCode" type="text" class="input3" id="CPCode" value="<?php echo $CPCode ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      ...
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$Code=trim($rowP['Code']);
							$PositionName=trim($rowP['PositionName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $Code ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $PositionName ?></td>
