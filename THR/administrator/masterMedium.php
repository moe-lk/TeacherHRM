...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$MediumSrc=$_REQUEST['MediumSrc'];
	$sqlSrch="SELECT * FROM CD_Medium where Medium!=''";  
	if($MediumSrc)$sqlSrch.=" and Medium like '%$MediumSrc%'";
...
if(isset($_POST["FrmSubmit"])){	
	...
	$Code=$_REQUEST['Code'];
	$Medium=trim($_REQUEST['Medium']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
	/* if ($Code == "") {
        $msg.= "Please enter the Medium Code.<br>";
    } */
    if ($Medium == "") {
        $msg.= "Please enter the Medium Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_Medium where Code='$Code'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Medium Code.<br>";
				}else{
					$queryMainSave = "INSERT INTO CD_Medium
					   (Code,Medium,RecordLog)
				 VALUES
					   ('$Code','$Medium','$RecordLog')";
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Medium SET Code='$Code',Medium='$Medium',RecordLog='$RecordLog' WHERE Code='$Code'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Medium Name :</td>
                      <td width="36%"><input name="MediumSrc" type="text" class="input2_n" id="MediumSrc" value="<?php echo $MediumSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td>Medium Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="Medium" type="text" class="input2" id="Medium" value="<?php echo $Medium ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
