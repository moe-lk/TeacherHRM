...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$DSNameSrc=$_REQUEST['DSNameSrc'];
	$sqlSrch="SELECT * FROM CD_DSec where DSName!=''";  
	if($DSNameSrc)$sqlSrch.=" and DSName like '%$DSNameSrc%'";
	$sqlSrch.=" order by DSName asc";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_DSec where DSCode='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$DSCode = trim($rowE['DSCode']);
	$DSName = trim($rowE['DSName']);
...
if(isset($_POST["FrmSubmit"])){	
	...
	if($msg==''){
		if($addEdit=='A'){
			$countSql="SELECT * FROM CD_DSec where DSCode='$DSCode'";
			$isAvailable=$db->rowAvailable($countSql);
			if($isAvailable==1){
				$msg.= "Duplicate Censes Code.<br>";
			}else{
				$queryMainSave = "INSERT INTO CD_DSec
			   (DSCode,DSName,DistName)
		 VALUES
			   ('$DSCode','$DSName','$DistName')";
				$db->runMsSqlQuery($queryMainSave);	
			}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_DSec SET DSCode='$DSCode',DSName='$DSName', DistName='$DistName' WHERE DSCode='$DSCode'";
			$db->runMsSqlQuery($queryMainUpdate);
		}
	}
	$fm="";
	$sqlSrch="SELECT * FROM CD_DSec where DSName!='' order by DSName asc";  
	$stmtP = $db->runMsSqlQuery($sqlSrch);
}
			  ...
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      ...
                      <td width="36%"><input name="DSNameSrc" type="text" class="input2_n" id="DSNameSrc" value="<?php echo $DSNameSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="DSCode" type="text" class="input3" id="DSCode" value="<?php echo $DSCode ?>" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      ...
                      <td>:</td>
                      <td><input name="DSName" type="text" class="input2" id="DSName" value="<?php echo $DSName ?>"/></td>
                    </tr>
                    <tr>
                      <td>District</td>
                      <td>:</td>
                      <td><select class="select2a_n" id="DistName" name="DistName">
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      ...
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$DSCode=trim($rowP['DSCode']);
							$DSName=trim($rowP['DSName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $DSCode ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $DSName ?></td>
                        <td bgcolor="#FFFFFF" align="center"><a href="<?php echo "$ttle-$pageid-$menu-$DSCode-E.html";?>">Click</a></td>
                      </tr>
                      <?php }?>
                    </table></td>
          </tr>
