...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$TitleNameSrc=$_REQUEST['TitleNameSrc'];
	$sqlSrch="SELECT * FROM CD_Title where TitleName!=''";  
	if($TitleNameSrc)$sqlSrch.=" and TitleName like '%$TitleNameSrc%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_Title where TitleCode='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$TitleCode = $rowE['TitleCode'];
	$TitleName = trim($rowE['TitleName']);
}
...
if(isset($_POST["FrmSubmit"])){	
    ...
    if ($TitleName == "") {
        $msg.= "Please enter the Title Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_Title where TitleCode='$TitleCode'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Title Code.<br>";
				}else{
					$queryMainSave = "INSERT INTO CD_Title
					   (TitleCode,TitleName,RecordLog)
				 VALUES
					   ('$TitleCode','$TitleName','$RecordLog')";
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Title SET TitleCode='$TitleCode',TitleName='$TitleName',RecordLog='$RecordLog' WHERE TitleCode='$TitleCode'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Title Name :</td>
                      <td width="36%"><input name="TitleNameSrc" type="text" class="input2_n" id="TitleNameSrc" value="<?php echo $TitleNameSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="TitleCode" type="text" class="input3" id="TitleCode" value="<?php echo $TitleCode ?>" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      <td>Title Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="TitleName" type="text" class="input2" id="TitleName" value="<?php echo $TitleName ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Title Name</td>
                        <td width="20%" align="center" bgcolor="#999999">Modify</td>
                      </tr>
                      <?php 
					  $i=1;
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$TitleCode=trim($rowP['TitleCode']);
							$TitleName=trim($rowP['TitleName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $TitleCode ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $TitleName ?></td>
                        <td bgcolor="#FFFFFF" align="center"><a href="<?php echo "$ttle-$pageid-$menu-$TitleCode-E.html";?>">Click</a></td>
                      </tr>
                      <?php }?>
                    </table></td>
          </tr>
