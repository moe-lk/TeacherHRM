...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$ServiceNameSrc=$_REQUEST['ServiceNameSrc'];
	$sqlSrch="SELECT * FROM CD_Service where ServiceName!=''";  
	if($ServiceNameSrc)$sqlSrch.=" and ServiceName like '%$ServiceNameSrc%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_Service where ServCode='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$ServCode = $rowE['ServCode'];
	$ServiceName = trim($rowE['ServiceName']);
}
...
if(isset($_POST["FrmSubmit"])){	
    ...
    if ($ServiceName == "") {
        $msg.= "Please enter the Service Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_Service where ServCode='$ServCode'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Service Code.<br>";
				}else{
					$queryMainSave = "INSERT INTO CD_Service
					   (ServCode,ServiceName,RecordLog)
				 VALUES
					   ('$ServCode','$ServiceName','$RecordLog')";
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Service SET ServCode='$ServCode',ServiceName='$ServiceName',RecordLog='$RecordLog' WHERE ServCode='$ServCode'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Service Name :</td>
                      <td width="36%"><input name="ServiceNameSrc" type="text" class="input2_n" id="ServiceNameSrc" value="<?php echo $ServiceNameSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="ServCode" type="text" class="input3" id="ServCode" value="<?php echo $ServCode ?>" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      <td>Service Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="ServiceName" type="text" class="input2" id="ServiceName" value="<?php echo $ServiceName ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Service Name</td>
                        <td width="20%" align="center" bgcolor="#999999">Modify</td>
                      </tr>
                      <?php 
					  $i=1;
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$ServCode=trim($rowP['ServCode']);
							$ServiceName=trim($rowP['ServiceName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $ServCode ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $ServiceName ?></td>
                        <td bgcolor="#FFFFFF" align="center"><a href="<?php echo "$ttle-$pageid-$menu-$ServCode-E.html";?>">Click</a></td>
                      </tr>
                      <?php }?>
                    </table></td>
          </tr>
