...
if(isset($_POST["FrmSrch"]) || $fm==''){
	...
	$sqlSrch="SELECT * FROM CD_Zone where InstitutionName!=''";  
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_Zone where CenCode='$id'";  
...
if(isset($_POST["FrmSubmit"])){	
	...
	if($msg==''){
		if($addEdit=='A'){
			$countSql="SELECT * FROM CD_Zone where CenCode='$CenCode'";
			$isAvailable=$db->rowAvailable($countSql);
			if($isAvailable==1){
				$msg.= "Duplicate Zone Code.<br>";
			}else{
				$queryMainSave = "INSERT INTO CD_Zone
			   (CenCode,InstitutionName,DistrictCode)
		 VALUES
			   ('$CenCode','$InstitutionName','$DistrictCode')";
				$db->runMsSqlQuery($queryMainSave);	
			}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Zone SET CenCode='$CenCode',InstitutionName='$InstitutionName', DistrictCode='$DistrictCode' WHERE CenCode='$CenCode'";
...
<table width="100%" cellpadding="0" cellspacing="0">
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td>Zone Name<span class="form_error_sched"> *</span></td>
                      <td>:</td>
                    ...
                    <tr>
                      <td>District</td>
                      <td>:</td>
                      <td><select class="select2a_n" id="DistrictCode" name="DistrictCode">
