...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$SubjectNameSrc=$_REQUEST['SubjectNameSrc'];
	$sqlSrch="SELECT * FROM CD_Subject where SubjectName!=''";  
	if($SubjectNameSrc)$sqlSrch.=" and SubjectName like '%$SubjectNameSrc%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_Subject where SubCode='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$SubCode = trim($rowE['SubCode']);
	$SubjectName = trim($rowE['SubjectName']);
}
...
if(isset($_POST["FrmSubmit"])){	
    ...
    if ($SubjectName == "") {
        $msg.= "Please enter the Subject Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_Subject where SubCode='$SubCode'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Subject Code.<br>";
				}else{
				$queryMainSave = "INSERT INTO CD_Subject
				   (SubCode,SubjectName,RecordLog)
			 VALUES
				   ('$SubCode','$SubjectName','$RecordLog')";
				$db->runMsSqlQuery($queryMainSave);
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_Subject SET SubCode='$SubCode',SubjectName='$SubjectName',RecordLog='$RecordLog' WHERE SubCode='$SubCode'";
			  ...
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      ...
                      <td width="36%"><input name="SubjectNameSrc" type="text" class="input2_n" id="SubjectNameSrc" value="<?php echo $SubjectNameSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="SubCode" type="text" class="input3" id="SubCode" value="<?php echo $SubCode ?>" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      <td>Subject Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="SubjectName" type="text" class="input2" id="SubjectName" value="<?php echo $SubjectName ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Subject Name</td>
                        <td width="20%" align="center" bgcolor="#999999">Modify</td>
                      </tr>
                      <?php 
					  $i=1;
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$SubCode=trim($rowP['SubCode']);
							$SubjectName=trim($rowP['SubjectName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $SubCode ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $SubjectName ?></td>
                        <td bgcolor="#FFFFFF" align="center"><a href="<?php echo "$ttle-$pageid-$menu-$SubCode-E.html";?>">Click</a></td>
                      </tr>
                      <?php }?>
                    </table></td>
          </tr>
