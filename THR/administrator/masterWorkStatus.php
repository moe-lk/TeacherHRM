...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$WorkStatusSrc=$_REQUEST['WorkStatusSrc'];
	$sqlSrch="SELECT * FROM CD_WorkStatus where WorkStatus!=''";  
	if($WorkStatusSrc)$sqlSrch.=" and WorkStatus like '%$WorkStatusSrc%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_WorkStatus where Code='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$Code = trim($rowE['Code']);
	$WorkStatus = trim($rowE['WorkStatus']);
}
...
if(isset($_POST["FrmSubmit"])){	
	...
	$Code=$_REQUEST['Code'];
	$WorkStatus=trim($_REQUEST['WorkStatus']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
    if ($WorkStatus == "") {
        $msg.= "Please enter the Work Status Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
			$countSql="SELECT * FROM CD_WorkStatus where Code='$Code'";
			$isAvailable=$db->rowAvailable($countSql);
			if($isAvailable==1){
				$msg.= "Duplicate Work Status Code.<br>";
			}else{
				$queryMainSave = "INSERT INTO CD_WorkStatus
				   (Code,WorkStatus,RecordLog)
			 VALUES
				   ('$Code','$WorkStatus','$RecordLog')";
				$db->runMsSqlQuery($queryMainSave);	
			}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_WorkStatus SET Code='$Code',WorkStatus='$WorkStatus',RecordLog='$RecordLog' WHERE Code='$Code'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Work Status Name :</td>
                      <td width="36%"><input name="WorkStatusSrc" type="text" class="input2_n" id="WorkStatusSrc" value="<?php echo $WorkStatusSrc ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td>Work Status Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="WorkStatus" type="text" class="input2" id="WorkStatus" value="<?php echo $WorkStatus ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Work Status Name</td>
