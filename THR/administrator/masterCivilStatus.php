...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$CivilStatusName=$_REQUEST['CivilStatusName'];
	$sqlSrch="SELECT * FROM CD_CivilStatus where CivilStatusName!=''";  
	if($CivilStatusName)$sqlSrch.=" and CivilStatusName like '%$CivilStatusName%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_CivilStatus where Code='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$Code = $rowE['Code'];
	$CivilStatusName = trim($rowE['CivilStatusName']);
...
if(isset($_POST["FrmSubmit"])){	
	...
	$Code=$_REQUEST['Code'];
	$CivilStatusName=trim($_REQUEST['CivilStatusName']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
    if ($CivilStatusName == "") {
        $msg.= "Please enter the Status Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$queryMainSave = "INSERT INTO CD_CivilStatus
				   (Code,CivilStatusName,RecordLog)
			 VALUES
				   ('$Code','CivilStatusName','$RecordLog')";
				$db->runMsSqlQuery($queryMainSave);	
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_CivilStatus SET Code='$Code',CivilStatusName='$CivilStatusName',RecordLog='$RecordLog' WHERE Code='$Code'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Civil Status Name :</td>
                      <td width="36%"><input name="CivilStatusName" type="text" class="input2_n" id="CivilStatusName" value="<?php echo $CivilStatusName ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td>Civil Status Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="CivilStatusName" type="text" class="input2" id="CivilStatusName" value="<?php echo $CivilStatusName ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Civil Status Name</td>
                      ...
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$Code=trim($rowP['Code']);
							$CivilStatusName=trim($rowP['CivilStatusName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $Code ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $CivilStatusName ?></td>
