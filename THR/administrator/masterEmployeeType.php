...
if(isset($_POST["FrmSrch"]) || $fm==''){
	$APPTypeName=$_REQUEST['APPTypeName'];
	$sqlSrch="SELECT * FROM CD_EmpType where APPTypeName!=''";  
	if($APPTypeName)$sqlSrch.=" and APPTypeName like '%$APPTypeName%'";
...
if($fm=='E'){
	$sqlSrch="SELECT * FROM CD_EmpType where APPCode='$id'";  
	$stmtE= $db->runMsSqlQuery($sqlSrch);
	$rowE = sqlsrv_fetch_array($stmtE, SQLSRV_FETCH_ASSOC);
	$APPCode = $rowE['APPCode'];
	$APPTypeName = trim($rowE['APPTypeName']);
...
if(isset($_POST["FrmSubmit"])){	
	...
	$APPTypeName=trim($_REQUEST['APPTypeName']);
	$dateU=date('Y-m-d H:i:s');
	if($addEdit=="A")$RecordLog="Add by $NICUser on $dateU";
	if($addEdit=="E")$RecordLog="Edit by $NICUser on $dateU";
	
    if ($APPTypeName == "") {
        $msg.= "Please enter the Employee Type Name.<br>";
    }
	if($msg==''){
		if($addEdit=='A'){
				$countSql="SELECT * FROM CD_EmpType where APPCode='$APPCode'";
				$isAvailable=$db->rowAvailable($countSql);
				if($isAvailable==1){
					$msg.= "Duplicate Employee Type Code.<br>";
				}else{
					$queryMainSave = "INSERT INTO CD_EmpType
					   (APPCode,APPTypeName,RecordLog)
				 VALUES
					   ('$APPCode','$APPTypeName','$RecordLog')";
					$db->runMsSqlQuery($queryMainSave);	
				}
		}else if($addEdit=='E'){
			$queryMainUpdate = "UPDATE CD_EmpType SET APPCode='$APPCode',APPTypeName='$APPTypeName',RecordLog='$RecordLog' WHERE APPCode='$APPCode'";
...
<table width="100%" cellpadding="0" cellspacing="0">
			  <tr>
			    <td valign="top" style="border-bottom:1px; border-bottom-style:solid;"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      <td width="24%">Employee Type Name :</td>
                      <td width="36%"><input name="APPTypeName" type="text" class="input2_n" id="APPTypeName" value="<?php echo $APPTypeName ?>"/></td>
	  ...
	  <tr>
                  ...
                  <table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td width="73%">
                      <input name="APPCode" type="text" class="input3" id="APPCode" value="<?php echo $APPCode ?>" <?php if($fm=='E'){?>readonly="readonly"<?php }?>/>
                    ...
                    <tr>
                      <td>Employee Type  Name<span class="form_error_sched">*</span></td>
                      <td>:</td>
                      <td><input name="APPTypeName" type="text" class="input2" id="APPTypeName" value="<?php echo $APPTypeName ?>"/></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                ...
                <tr>
                    <td bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                      <tr>
                        ...
                        <td width="56%" align="center" bgcolor="#999999">Employee Type Name</td>
                        <td width="20%" align="center" bgcolor="#999999">Modify</td>
                      </tr>
                      <?php 
					  $i=1;
                      while ($rowP = sqlsrv_fetch_array($stmtP, SQLSRV_FETCH_ASSOC)) {
							$APPCode=trim($rowP['APPCode']);
							$APPTypeName=trim($rowP['APPTypeName']);
					  ?>
                      <tr>
                        <td height="20" bgcolor="#FFFFFF"><?php echo $i++ ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $APPCode ?></td>
                        <td bgcolor="#FFFFFF"><?php echo $APPTypeName ?></td>
                        <td bgcolor="#FFFFFF" align="center"><a href="<?php echo "$ttle-$pageid-$menu-$APPCode-E.html";?>">Click</a></td>
                      </tr>
                      <?php }?>
                    </table></td>
          </tr>
