	...
	if($msg==''){
		if($pMastStatus=='Add'){
			$queryMainSave = "INSERT INTO ArchiveUP_TeacherMast
			   (Title,SurnameWithInitials,FullName,DOB,EthnicityCode,GenderCode,ReligionCode,emailaddr,MobileTel,LastUpdate,RecStatus,UpdateBy,RecordLog)
		 VALUES
			   ('$Title','$SurnameWithInitials','$FullName','$DOB','$EthnicityCode','$GenderCode','$ReligionCode','$emailaddr','$MobileTel','$LastUpdate','$RecStatus','$NICUser','First change')";
            $db->runMsSqlQuery($queryMainSave);	
		}else if($perAddStatus=='Update'){
			$queryMainUpdate = "UPDATE ArchiveUP_StaffAddrHistory SET Title='$Title',SurnameWithInitials='$SurnameWithInitials',FullName='$FullName',DOB='$DOB',EthnicityCode='$EthnicityCode',GenderCode='$GenderCode',ReligionCode='$ReligionCode',emailaddr='$emailaddr',MobileTel='$MobileTel',LastUpdate='$LastUpdate',UpdateBy='$NICUser',RecordLog='Edit record' WHERE NIC='$NICUser' and RecStatus='0'";
	...
	if($msg==''){
		if($curAddStatus=='Add'){
		 ...
		 VALUES
			...
			$queryMainUpdate = "UPDATE ArchiveUP_StaffAddrHistory SET Address='$AddressC',DSCode='$DSCodeC',DISTCode='$DISTCodeC',Tel='$TelC',AppDate='$AppDateC',LastUpdate='$LastUpdate',UpdateBy='$NICUser',RecordLog='Edit record' WHERE NIC='$NICUser' and AddrType='CUR'";
			   
            $db->runMsSqlQuery($queryMainUpdate);
		}
	}
	if($msg=='')$success="Your update request submitted successfully. Data will be displaying after the approvals.";
	//exit();
...
if($menu=='E'){
	...
	FROM            ArchiveUP_StaffAddrHistory INNER JOIN
							 CD_DSec ON ArchiveUP_StaffAddrHistory.DSCode = CD_DSec.DSCode INNER JOIN
							 CD_Districts ON ArchiveUP_StaffAddrHistory.DISTCode = CD_Districts.DistCode
	WHERE        (ArchiveUP_StaffAddrHistory.NIC = '$NICUser') AND (ArchiveUP_StaffAddrHistory.AddrType = N'CUR')";//538093300V
...
WHERE        (ArchiveUP_TeacherMast.NIC = N'$NICUser') AND (ArchiveUP_TeacherMast.RecStatus = N'0')";//538093300V
                  ...
                  <script type="text/javascript">
                    ...
                    <td width="50%" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                            ...
                            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                ...
                                echo "<option value=\"$TitleCodedb\" $seltebr>$EthnicityNamed</option>";
                            }
                            ?>
                              </select></td>
                            </tr>
                            ...
                            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                ...
                                echo "<option value=\"$TitleCodedb\" $seltebr>$ReligionNamed</option>";
                            }
                            ?>
                              </select></td>
                            </tr>
                ...
                <tr>
                  <td colspan="2" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                    ...
                    <tr>
                      ...
                      <td align="left" valign="top"><strong>DS Division</strong></td>
                      <td align="left" valign="top"><strong>:</strong></td>
                      <td align="left" valign="top"><?php //echo $DSNameC ?><div id="txt_divisionC"><select class="select2a_n" id="DSCodeC" name="DSCodeC">
                                  <!--<option value="">School Name</option>-->
                                  <?php
                            $sql = "SELECT DSCode,DSName FROM CD_DSec where DistName='$DistCode' order by DSName asc";
                            $stmt = $db->runMsSqlQuery($sql);
