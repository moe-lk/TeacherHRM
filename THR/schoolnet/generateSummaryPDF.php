...
if (isset($_REQUEST["groupBy"])) {
    ...
    foreach ($arryCheck as $value) {
		...
		if ($value == "Religion") {
            $arrField[] = "CD_Religion.ReligionName AS religion";
            $arrGroupField[] = "CD_Religion.ReligionName";
            $arrHeader[] = "Religion";
        }
		if ($value == "Civil Status") {
            $arrField[] = "CD_CivilStatus.CivilStatusName AS religion";
            $arrGroupField[] = "CD_CivilStatus.CivilStatusName";
            $arrHeader[] = "Civil Status";
        }
		if($value == "Status"){ // Added School status
            $arrField[] = "CD_CensesNo.SchoolStatus AS status";
            $arrGroupField[] = "CD_CensesNo.SchoolStatus";
            $arrHeader[] = "Status";
        }
        if($value == "Type"){
            $arrField[] = "CD_CensesCategory.Category";// Add status details here
            $arrOrder[] = "CD_CensesCategory.Category";
            $arrHeader[] = "School Type";
        }
        if ($value == "Qualification") {
            $arrField[] = "CD_QualificationCategory.Description";
            $arrOrder[] = "CD_QualificationCategory.Description";
            $arrHeader[] = "Qualification";
        }
        if ($value == "Category") {
...
$html.="<div style=\"text-align:center; height:200px; width:auto;\"><p style=\"font-size:28px; font-weight:600;\">Ministry of Education Sri Lanka<br>Teacher Human Resource Management Portal - NEMIS </p><p style=\"font-size:20px; font-weight:600;\">". $txtRptHedding ."<br>Summary Report&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . date('Y-m-d H:i:s') . "</p></div>";
...
$html.="<div style=\"height:150px;\"><b>
 School Type :" . $schType . "</br>Province : " . $provinceName . "</br>District : " . $districtName . "</br>Zone : " . $zoneName . "</br>Division : " . $divisionName . "</br>School : " . $schoolName . " </br>School Status : " . $schoolStatus ."</b></div>";
...
if (!empty($arrQulificationCode)) {
  ...
  CD_QualificationCategory.Description AS qualification
FROM CD_Qualif Inner Join CD_QualificationCategory 
ON CD_QualificationCategory.Code = CD_Qualif.Category
WHERE (Code IN ('" . implode("','", $arrQulificationCode) . "'))";
    $stmtBio = $db->runMsSqlQuery($sqlBio);
...
Inner Join CD_QualificationCategory 
    ON CD_QualificationCategory.Code = CD_Qualif.Category ";
}
...
$html.="<tr>";
    $html.="<td width=\"10%\" align=\"center\" style=\"border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000; border-top: 1px solid #000000;\"><b>Total</b></td><td width=\"10%\" colspan=\"".count($arrHeader)."\" align=\"right\" style=\"border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-top: 1px solid #000000; border-right:1px solid #000000;\">" . number_format($total) . "</td> ";
    $html.="</tr>";
$html.="</table>";
// var_dump($_REQUEST["txtQuliName"]);
/*
  echo $sql;
  if ($stmt) {
  $a = 0;
  ...
  echo "<div><b>Total No of Heades : " . number_format($totalCount) . "</b></div>";
 */
