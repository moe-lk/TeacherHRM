...
if (isset($_REQUEST["selectColum"])) {
    ...
    foreach ($arryColum as $value) {
        ...
        if ($value == "Teaching") {
            $arrField[] = "TG_TeachingTemp.SubjectName";
...
if (!empty($arrQulificationCode)) {
    $sqlBio = "SELECT Description AS qualification FROM MOENational.dbo.CD_Qualif WHERE (Qcode IN ('" . implode("','", $arrQulificationCode) . "'))";
    $stmtBio = $db->runMsSqlQuery($sqlBio);
...
if (isset($_REQUEST["txtQuliName"])) {
    $sql.= " INNER JOIN StaffQualification ON TeacherMast.HQualificatinRef = StaffQualification.ID INNER JOIN CD_Qualif ON StaffQualification.QCode = CD_Qualif.Qcode";
}
...
//$sql.= " AND (StaffServiceHistory.ServiceRecTypeCode IS NULL or (StaffServiceHistory.ServiceRecTypeCode != 'DS03' AND StaffServiceHistory.ServiceRecTypeCode != 'RN01' AND StaffServiceHistory.ServiceRecTypeCode != 'RT01' AND StaffServiceHistory.ServiceRecTypeCode != 'DS01'))";
