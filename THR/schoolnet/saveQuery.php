...
$cmbSchoolStatus = $_REQUEST["cmbSchoolStatus"]; // Added school status 
// Query save
...
ORDER BY ID DESC";
        ...
        //Added the school status 
        if(!empty($cmbSchoolStatus)){
            $sqlInsert = "INSERT INTO TG_QuerySaveGeography VALUES('$sequenceID','SS','$cmbSchoolStatus')";
            $stmt = $db->runMsSqlQuery($sqlInsert);
        }
        ...
        if (isset($_REQUEST["txtTeachType"])) {
            ...
            for ($i = 0; $i < count($arrTeachType); $i++) {
                $sqlInsert = "INSERT INTO TG_QuerySaveTeaching VALUES('$sequenceID','$arrTeachType[$i]','$arrTeachSubject[$i]', '$arrTeachMedium[$i]','$arrTeachGrade[$i]')";
                $stmt = $db->runMsSqlQuery($sqlInsert);
            }
        }
