...
/* $sqlStr="Select SubjectName from CD_Subject where SubjectName!=''";
	$stmtSub = $db->runMsSqlQuery($sqlStr);
	$rowSub= sqlsrv_fetch_array($stmtSub, SQLSRV_FETCH_ASSOC);
		echo $restValue=$rowSub['SubjectName']; */
...
FROM            TG_StaffLeave INNER JOIN
						 ...
						 where TG_StaffLeave.NIC='$NICUser' and TG_StaffLeave.ApplyBy='SELF'";//$NICUser
...
if(isset($_POST["FrmSubmit"])){
	...
	//exit();
		$queryGradeSave="INSERT INTO $tblNam
			   (NIC,ServiceRecRef,LeaveType,StartDate,EndDate,LastUpdate,UpdateBy,Reference,RecordLog,NoofDays,AttachFile,IsApproved,ApplyBy,SchoolID)
		 VALUES
			   ('$NIC','$ServiceRecRef','$LeaveType','$StartDate','$EndDate','$LastUpdate','$UpdateBy','$Reference','$RecordLog','$NoofDays','$fileSaveName','$IsApproved','$ApplyBy','$SchoolID')";
	...
	FROM $tblNam
	WHERE (NIC = N'$NIC') and (StartDate='$StartDate') and (EndDate='$EndDate')";

			$resMax = $db->runMsSqlQuery($sqlMax);
			$rowMax = sqlsrv_fetch_array($resMax, SQLSRV_FETCH_ASSOC);
			$requestID = $rowMax['rRequestMaxID'];    */
			$processType = $LeaveType;

			$msg = getApproveListLeave($processType, $newID);
...
<div class="main_content_inner_block">
  ...
  where NIC='$NICUser'";
                    ...
                    <tr>
                      ...
                      <td><input name="NoofDays" type="text" class="input3new" id="NoofDays" /></td>
                    </tr>
                    <tr>
                      <td>Strat Date :</td>
                      <td>
        ...
        <td width="44%" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                <tr>
                  ...
                  <!-- Removed the Text Box which tends to edit, by Dharshana -->
                  <td><h4><?php echo $loggedPositionName?></h4></td>
                </tr>
                <tr>
                  <td align="left" valign="top">1st Appoinment Date :</td>
                  <!-- Removed the Text Box which tends to edit, by Dharshana -->
									<td><h4><?php echo $firstAppDate?></h4></td>
                </tr>
                <tr>
                  <td align="left" valign="top">Remarks :</td>
                  <td rowspan="2" valign="top"><textarea name="Reference" cols="45" rows="3" class="textarea1auto" id="Reference"></textarea></td>
                </tr>
                <tr>
  ...
  $i=1;
   $stmt = $db->runMsSqlQuery($countTotal);
                      ...
                      <tr>
                        ...
                        <td bgcolor="#FFFFFF" align="center"><a href="javascript:aedWin('<?php echo $Expr1 ?>','D','Leave','<?php echo $tblNam ?>','<?php echo "$ttle-$pageid.html";?>')">Delete <?php //echo $Expr1 ?></a></td>
                      </tr>
                      <?php }?>
                      <tr>
                        <td bgcolor="#FFFFFF">&nbsp;</td>
