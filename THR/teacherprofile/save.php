...
WHERE
	...
	FROM            TeacherMast INNER JOIN
        ...
        if($AccessLevel==""){
            $msg1 .= "User access level not assign for this user.<br>";
        }

        if ($msg1) {
            $_SESSION['success_update'] = "Error(s) on the page. <br>" . "" . $msg1;
...
WHERE
	(
		CD_CensesNo.CenCode = '$InstCodeCurrent'
	)";
        ...
        if ($fmRec == '') {
            // ** For new record entering **
            $queryMainSave = "INSERT INTO UP_StaffServiceHistory (
	NIC,
	ServiceRecTypeCode,
	AppDate,
	InstCode,
...
VALUES
	(
		...
		'N'
            ...
            // **  For record update **
            if ($menuRec == 'U') {
                $queryMainUpdate = "INSERT INTO UP_StaffServiceHistory (
	NIC,
	ServiceRecTypeCode,
	AppDate,
	InstCode,
...
VALUES
	(
		...
		'N'
                ...
                audit_trail($NICUser, $_SESSION["NIC"], 'teacherprofile\service.php', 'Insert', 'UP_StaffServiceHistory,TG_Approval', 'Insert user services. From update. For direct approve');
            } else {
                if ($menuRec == 'U') {
			 ...
			 VALUES
				   ('$NICUser','$ServiceRecTypeCode','$AppDate','$InstCode','$SecGRCode','','$ServiceTypeCode','','$PositionCode','$Cat2003Code','$Reference','','$LastUpdate','$updateBy','update by $updateBy','$fmRec','N')";
                    ...
                    audit_trail($NICUser, $_SESSION["NIC"], 'teacherprofile\service.php', 'Insert', 'UP_StaffServiceHistory,TG_Approval', 'Insert user services. For direct approve');
                } else {
                    ...
                    audit_trail($NICUser, $_SESSION["NIC"], 'teacherprofile\service.php', 'Update', 'UP_StaffServiceHistory,TG_Approval', 'Update pending user services. For direct approve');
                }
        ...
        // ** Insert dadta for approval process **
...
TG_ServiceApprovalCycle.TransferType = 'ZZ'";
        ...
        while ($rowAC = sqlsrv_fetch_array($stmtAC, SQLSRV_FETCH_ASSOC)) {
            ...
            if ($rowAC['AccessRoleType'] == "PC") {
                // ** for principle system auto update **

                if ($AccessLevel == '1000') {
                    // ** check service is belog to a teacher
