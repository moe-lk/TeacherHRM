...
//echo "<br>";
 ...
 // echo "11111";
//echo "<br>";
//echo $_SESSION["DivisionCodeU"];
//exit();
    if ($AccessRoleType == "SC") {
        
    } else if ($AccessRoleType == "ED") {
        if ($DivisionCodeU != $_SESSION["DivisionCodeU"]) {
            $_SESSION['success_update'] = "You don't have permission";
            header("Location:../teacherprofile/");
        }
    } else if ($AccessRoleType == "ZN") {
        if ($ZoneCodeU != $_SESSION["ZoneCodeU"]) {
            $_SESSION['success_update'] = "You don't have permission";
            header("Location:../teacherprofile/");
        }
    } else if ($AccessRoleType == "DN") {
        if ($DistCodeU != $_SESSION["DistCodeU"]) {
            $_SESSION['success_update'] = "You don't have permission";
            header("Location:../teacherprofile/");
        }
    } else if ($AccessRoleType == "PD") {
        if ($ProCodeU != $_SESSION["ProCodeU"]) {
            $_SESSION['success_update'] = "You don't have permission";
            header("Location:../teacherprofile/");
        }
    } else {
...
dbo.TG_DynMenu.ShowMenu = 1";
                                                ...
                                                while ($rowC = sqlsrv_fetch_array($stCh, SQLSRV_FETCH_ASSOC)) {
                                                    ...
                                                    if (str_replace("-tab", "", basename($_SERVER['REQUEST_URI'])) == $url) {
                                                        ...
                                                        if ($count == 1 && basename($_SERVER['REQUEST_URI']) == "teacherprofile") {
                                                            $active_class = "active";
                                                        }
                                                    }
                                                    ?>
