...
if ($RequestType == "getQuliDetails") { // Get qualification details from form
    $str = "";
...
if ($RequestType == "getTecDetails") { // Teaching section details
    $str = "";
...
if ($RequestType == "getSerDetails") { // Service details section
    $str = "";
    $str.= "<tr>";
    $str.= "<td><select name=\"cmbPosition[]\"><option value=\"\">-Select-</option>";
    $sql = "SELECT Code, PositionName FROM CD_Positions WHERE (Code <> N'') ORDER BY PositionName";
    ...
    $sql = "SELECT ServCode, LTRIM(ServiceName) AS serviceName FROM CD_Service WHERE (ServCode <> N'')";
    $stmt = $db->runMsSqlQuery($sql);
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
...
ORDER BY WorkStatus";
    ...
    $str.= "<td><a href=\"javascript:void(0);\" onclick=\"addServiceRow();\"><img src=\"images/add.png\" width=\"14\" height=\"14\"/></a></td></tr>";
    echo $str;
}

if ($RequestType == "getQulification") { // Get Qualification details
    $level = $_GET["level"];
    $str = "";

    $str.= "<select name=\"cmbQulification[]\"><option value=\"\">-Select-</option>";
    $sql = "SELECT Qcode, Description FROM CD_Qualif WHERE (Qcode != '') AND (Level = N'" . $level . "') ORDER BY Description";
    $stmt = $db->runMsSqlQuery($sql);
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        $str.="<option value=" . $row["Qcode"] . ">" . $row["Description"] . "</option>";
    }
...
if ($RequestType == "getDataAccordingToSCType") { //Get according to school type 
...
// Edited by NHVithana // Edited by NHVithana // Edited by NHVithana // Edited by NHVithana // Edited by NHVithana

// var_dump($cmbSchoolType);
if($RequestType == "getDataAccordingToSCStatus"){ // Get according to school status  
    $cmbSchoolType = $_POST["cmbSchoolType"];
    $cmbProvince = $_POST["cmbProvince"];
    $cmbDistrict = $_POST["cmbDistrict"];
    $cmbZone = $_POST["cmbZone"];
    $cmbDivision = $_POST["cmbDivision"];
    $cmbSchoolStatus = $_POST["cmbSchoolStatus"]; // Added school status
    $LOGGEDUSERID = $nicNO; 
    ...
    if ($cmbSchoolType == "")
        $cmbSchoolType = null;
    if ($cmbSchoolStatus == ""){ // Added school status
    ...
    $stmt = $db->runMsSqlQuery($sql, $params1);
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        $dataSchool.= '<option value=' . $row['CenCode'] . '>' . $row['SchoolStatus'] . '</option>';
    }
    //echo $dataSchool;
    $result = array();
    $result[0] = $dataSchool;
...
echo("This");
// start load according to school status
if ($RequestType == 'getDataAccordingToSchoolStatus') {
    $cmbSchoolType = $_POST["cmbSchoolType"];
    $cmbProvince = $_POST["cmbProvince"];
    $cmbDistrict = $_POST["cmbDistrict"];
    $cmbZone = $_POST["cmbZone"];
...
//end load according to school status
...
if ($RequestType == 'getBiographicalDetail') {
    ...
    switch ($bioItemVal) {
        case "G":
            $sql = "SELECT GenderCode AS feildCode, [Gender Name] AS feildName FROM CD_Gender WHERE (GenderCode <> N'')";
            break;
        case "E":
            $sql = "SELECT Code AS feildCode, EthnicityName AS feildName FROM CD_nEthnicity WHERE (Code <> N'')";
            break;
        case "R":
            $sql = "SELECT Code AS feildCode, ReligionName AS feildName FROM CD_Religion WHERE (Code <> N'')";
            break;
        case "C":
            $sql = "SELECT Code AS feildCode, CivilStatusName AS feildName FROM CD_CivilStatus WHERE (Code <> N'')";
            break;
    }
    $bioDeatails = array();
