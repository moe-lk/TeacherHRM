...
$approvSql = "SELECT        TG_Approval.id AS ReqAppID, TG_Approval.RequestID, TG_Approval.RequestType, TG_Approval.ApproveDesignationCode, 
                         ...
                         TG_IncrementRequest.IsApproved , CD_CensesNo.InstitutionName, TG_IncrementRequest.QuecAnswers
...
WHERE        (TG_Approval.RequestType = 'TeacherIncrement') AND (TG_Approval.$tblField = N'$accLevel') AND (TG_Approval.ApproveInstCode = N'$loggedSchool') AND (TG_Approval.ApprovedStatus = N'P')";
                ...
                <tr>
                    <td colspan="2" bgcolor="#CCCCCC"><table width="100%" cellspacing="1" cellpadding="1">
                            <tr>
                                ...
                                <td width="31%" align="center" bgcolor="#999999">Teacher Name</td>
                                <td width="26%" align="center" bgcolor="#999999">School</td>
                                <td width="14%" align="center" bgcolor="#999999">Effective Date</td>
                                <td width="17%" align="center" bgcolor="#999999">Apply Date</td>
                ...
                <tr>
                    <td colspan="2" ><table width="100%" cellspacing="1" cellpadding="1">
                            ...
                            <tr>
                                ...
                                <td width="22%" align="right">Mobile Number</td>
                                <td width="2%">:</td>
...
WHERE        (QualificationSubjects.QualificationID = '$Expr1')";
                            ...
                            <tr>
                                ...
                                <td valign="top">:</td>
                                <td colspan="4"><?php if ($AttachFile) { ?><a href="<?php echo "../hr/incrementattachments/$AttachFile"; ?>" target="_blank">View</a><?php } else {
                                    echo "Not Available";
                                } ?></td>
                            </tr>
                        </table></td>
...
FROM            TG_Approval INNER JOIN
						 ...
						 WHERE (TG_Approval.RequestType = 'TeacherIncrement') AND (TG_Approval.RequestID = '$id')
...
WHERE        (CD_AccessRoles.AccessRoleValue = '$ApproveDesignationCode') AND (CD_CensesNo.CenCode = N'$ApproveInstCode') AND (StaffServiceHistory.ServiceRecTypeCode != 'RT01' or StaffServiceHistory.ServiceRecTypeCode IS NULL)";
      ...
      ,[OrderID]
      ,[RecordLog]
  FROM [dbo].[CD_TG_IncrementQuestions] WHERE QuestionType='Teacher' ORDER BY OrderID ASC";
                                    $stmt = $db->runMsSqlQuery($sql);
                                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                        ...
                                        <tr>
                                            <td width="7%" height="25" align="center" bgcolor="#FFFFFF"><?php echo sprintf("%02d", $x);
                        ; ?>.</td>
        ...
        ?><?php //echo "-$ApproveDesignationCode-"; echo "<br>"; echo "-$accLevel-"; ?></td>
                                    ...
                                    <td width="1%">:</td>
                                    <td width="33%" rowspan="4" valign="top"><textarea name="ApproveComment" id="ApproveComment" cols="35" rows="6" <?php if ($activate == 'N') { ?>disabled="disabled"<?php } ?>><?php echo $Remarks ?></textarea></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td>:</td>
                                    <td><?php if ($ApprovedStatus == 'A' || $ApprovedStatus == 'R') {
            echo $DateTime;
        } else {
            echo date('Y-m-d');
        } ?></td>
                        ...
                        <td valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                                <tr>
                                    ...
                                    <td width="68%"><input type="hidden" name="ApID" value="<?php echo $ApID ?>" />
                                        ...
                                        <input type="hidden" name="ApprovedByName" value="<?php echo $SurnameWithInitialsED ?>" />
                                        <input type="hidden" name="RequestType" value="TeacherIncrement" />
