...
if (isset($_POST["FrmSubmit"])) {
	...
	//$sqlServiceRef = "SELECT CurServiceRef FROM TeacherMast WHERE NIC='$NICUser'";
...
WHERE        (TeacherMast.NIC = '$NICUser')";
    ...
    if ($Address == "") {
        $msg.= "Please enter permanant address.<br>";
    }
    if ($DSCode == "") {
        $msg.= "Please select permanant division.<br>";
    }
	if ($DISTCode == "") {
        $msg.= "Please select permanant district.<br>";
    }
	
	if($msg==''){
		if($PermResiIDA==''){//$perAddStatus=='Add'
			$queryMainSave = "INSERT INTO ArchiveUP_StaffAddrHistory
			   (NIC,AddrType,Address,DSCode,DISTCode,Tel,AppDate,LastUpdate,UpdateBy,RecordLog)
		 VALUES
			   ('$NICUser','PER','$Address','$DSCode','$DISTCode','$Tel','$AppDate','$LastUpdate','$NICUser','First change')";
            //$db->runMsSqlQuery($queryMainSave);	
			$PermResiID=$db->runMsSqlQueryInsert($queryMainSave);
		}else {//if($perAddStatus=='Update'){
			$queryMainUpdate = "UPDATE ArchiveUP_StaffAddrHistory SET Address='$Address',DSCode='$DSCode',DISTCode='$DISTCode',Tel='$Tel',AppDate='$AppDate',LastUpdate='$LastUpdate',UpdateBy='$NICUser',RecordLog='Edit record' WHERE ID='$PermResiIDA'";//NIC='$NICUser' and AddrType='PER'";
			   
            /* $db->runMsSqlQuery($queryMainUpdate);
			$sqlCA = "SELECT ID FROM ArchiveUP_StaffAddrHistory WHERE NIC='$NICUser' and AddrType='PER'";
            $stmtCA = $db->runMsSqlQuery($sqlCA);
            $row = sqlsrv_fetch_array($stmtCA, SQLSRV_FETCH_ASSOC);
			$PermResiID=trim($row['ID']); */
   ...
   //Current pw
    ...
    if ($AddressC == "") {
        $msg.= "Please enter current address.<br>";
    }
    if ($DSCodeC == "") {
        $msg.= "Please select current division.<br>";
    }
	if ($DISTCodeC == "") {
        $msg.= "Please select current district.<br>";
    }
	...
	if($msg==''){
		if($CurrResIDA==''){//$curAddStatus=='Add'
			$queryMainSave = "INSERT INTO ArchiveUP_StaffAddrHistory
			   (NIC,AddrType,Address,DSCode,DISTCode,Tel,AppDate,LastUpdate,UpdateBy,RecordLog)
		 VALUES
			   ('$NICUser','CUR','$AddressC','$DSCodeC','$DISTCodeC','$TelC','$AppDateC','$LastUpdate','$NICUser','First change')";
           // $db->runMsSqlQuery($queryMainSave);	
			$CurrResID=$db->runMsSqlQueryInsert($queryMainSave);
		}else {//if($curAddStatus=='Update'){
			$queryMainUpdate = "UPDATE ArchiveUP_StaffAddrHistory SET Address='$AddressC',DSCode='$DSCodeC',DISTCode='$DISTCodeC',Tel='$TelC',AppDate='$AppDateC',LastUpdate='$LastUpdate',UpdateBy='$NICUser',RecordLog='Edit record' WHERE ID='$CurrResIDA'";//NIC='$NICUser' and AddrType='CUR'";
			   
            $db->runMsSqlQuery($queryMainUpdate);
			
			/* $sqlCA = "SELECT ID FROM ArchiveUP_StaffAddrHistory WHERE NIC='$NICUser' and AddrType='CUR'";
            $stmtCA = $db->runMsSqlQuery($sqlCA);
            $row = sqlsrv_fetch_array($stmtCA, SQLSRV_FETCH_ASSOC);
			$CurrResID=trim($row['ID']); */
			//$CurrResID=$db->runMsSqlQueryInsert($queryMainUpdate);
	...
	if($msg==''){
		...
		if($TeacherMastIDA==''){
		 ...
		 VALUES
			   ('$NICUser','$Title','$SurnameWithInitials','$FullName','$DOB','$EthnicityCode','$PermResiID','$CurrResID','$CurServiceRef','$GenderCode','$ReligionCode','$emailaddr','$MobileTel','0','$LastUpdate','$NICUser','First change')";
            //$db->runMsSqlQuery($queryMainSave);	
			$TeacherMastID=$db->runMsSqlQueryInsert($queryMainSave);
			
		}else {//if($pMastStatus=='Update')
			$queryMainUpdate = "UPDATE ArchiveUP_TeacherMast SET Title='$Title',SurnameWithInitials='$SurnameWithInitials',FullName='$FullName',DOB='$DOB',EthnicityCode='$EthnicityCode',PerResRef='$PermResiID',CurResRef='$CurrResID',CurServiceRef='$CurServiceRef',GenderCode='$GenderCode',ReligionCode='$ReligionCode',emailaddr='$emailaddr',MobileTel='$MobileTel',LastUpdate='$LastUpdate',UpdateBy='$NICUser',RecordLog='Edit record' WHERE ID='$TeacherMastIDA'";//NIC='$NICUser' and 
	...
	if ($msg == '') {
		//$countSql="SELECT * FROM TG_EmployeeUpdatePersInfo where NIC='$NICUser' and IsApproved='N'";
		$isAvailable=$db->rowAvailable($sqlCAllready);
		if($isAvailable==1){
			
			$queryMainUpdate = "UPDATE TG_EmployeeUpdatePersInfo SET TeacherMastID='$TeacherMastID',PermResiID='$PermResiID',CurrResID='$CurrResID',ZoneCode='$ZoneCode',dDateTime='$LastUpdate',IsApproved='N',ApproveDate='',ApprovedBy='',UpdateBy='$NICUser' WHERE NIC='$NICUser' and IsApproved='N'";
			$db->runMsSqlQuery($queryMainUpdate);
			...
			$queryRegis = "INSERT INTO TG_EmployeeUpdatePersInfo				   (NIC,TeacherMastID,PermResiID,CurrResID,dDateTime,ZoneCode,IsApproved,ApproveComment,ApproveDate,ApprovedBy,UpdateBy)
			 VALUES				   
		('$NICUser','$TeacherMastID','$PermResiID','$CurrResID','$LastUpdate','$ZoneCode','N','','','','$NICUser')";
...
if($menu=='E'){
	...
	WHERE   ArchiveUP_StaffAddrHistory.ID = '$PermResiIDA'";//     (ArchiveUP_StaffAddrHistory.NIC = '$NICUser') AND (ArchiveUP_StaffAddrHistory.AddrType = N'PER')";
	...
	$sqlCurAdd="SELECT    ArchiveUP_StaffAddrHistory.Address, ArchiveUP_StaffAddrHistory.Tel, 
							 CONVERT(varchar(20),ArchiveUP_StaffAddrHistory.AppDate,121) AS AppDate, CD_DSec.DSName, CD_Districts.DistName, CD_DSec.DSCode, CD_Districts.DistCode
	FROM            ArchiveUP_StaffAddrHistory INNER JOIN
							 CD_DSec ON ArchiveUP_StaffAddrHistory.DSCode = CD_DSec.DSCode INNER JOIN
							 CD_Districts ON ArchiveUP_StaffAddrHistory.DISTCode = CD_Districts.DistCode
	WHERE  ArchiveUP_StaffAddrHistory.ID ='$CurrResIDA'";//(ArchiveUP_StaffAddrHistory.NIC = '$NICUser') AND (ArchiveUP_StaffAddrHistory.AddrType = N'CUR')";//538093300V
	...
	$sqlPmast="SELECT        ArchiveUP_TeacherMast.ID, ArchiveUP_TeacherMast.NIC, ArchiveUP_TeacherMast.SurnameWithInitials, ArchiveUP_TeacherMast.FullName, ArchiveUP_TeacherMast.Title, ArchiveUP_TeacherMast.MobileTel, CONVERT(varchar(20), 
                         ArchiveUP_TeacherMast.DOB, 121) AS DOB, CD_nEthnicity.EthnicityName, CD_Religion.ReligionName, CD_Gender.[Gender Name], ArchiveUP_TeacherMast.emailaddr, CD_Title.TitleName,
                          ArchiveUP_TeacherMast.GenderCode, ArchiveUP_TeacherMast.EthnicityCode, ArchiveUP_TeacherMast.ReligionCode
FROM            ArchiveUP_TeacherMast INNER JOIN
                         CD_Gender ON ArchiveUP_TeacherMast.GenderCode = CD_Gender.GenderCode INNER JOIN
                         CD_nEthnicity ON ArchiveUP_TeacherMast.EthnicityCode = CD_nEthnicity.Code INNER JOIN
                         CD_Religion ON ArchiveUP_TeacherMast.ReligionCode = CD_Religion.Code INNER JOIN
                         CD_Title ON ArchiveUP_TeacherMast.Title = CD_Title.TitleCode
WHERE  ArchiveUP_TeacherMast.ID='$TeacherMastIDA'";//     (ArchiveUP_TeacherMast.NIC = N'$NICUser') AND (ArchiveUP_TeacherMast.RecStatus = N'0')";//538093300V
        ...
        <div class="mcib_middle1">
            <table width="945" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td align="right" valign="top"><a href="personalInfo-1-E.html"><img src="../cms/images/udate-request.png" width="170" height="26" /></a></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td colspan="2" valign="top" style="border-bottom:1px; border-bottom-style:solid;"><strong>Permanant Residance Details</strong></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td colspan="2" valign="top" style="border-bottom:1px; border-bottom-style:solid;"><strong>Current Residance Details</strong></td>
                </tr>
                <tr>
                  <td colspan="2" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      <td width="15%" align="left" valign="top"><strong> Address</strong></td>
                      <td width="1%" align="left" valign="top"><strong>:</strong></td>
                      <td width="34%" rowspan="4" align="left" valign="top"><?php echo $AddressC ?></td>
                      <td width="19%" align="left" valign="top"><strong>District</strong></td>
                      <td width="1%" align="left" valign="top"><strong>:</strong></td>
...
<?php if($success==''){?>
            <table width="945" cellpadding="0" cellspacing="0">
                <tr>
                  <td valign="top"><span style="color:#090; font-weight:bold;">*If your personal data record is inaccurate, you can submit an update request</span></td>
                  <td align="right" valign="top"><a href="personalInfo-1.html"><img src="../cms/images/current-details.png" width="138" height="26" /></a></td>
                </tr>
                <tr>
                    <td width="50%" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                            <tr>
                ...
                <tr>
                  <td colspan="2" valign="top"><table width="100%" cellspacing="2" cellpadding="2">
                    <tr>
                      <td width="15%" align="left" valign="top"><strong> Address</strong></td>
                      <td width="1%" align="left" valign="top"><strong>:</strong></td>
                      <td width="34%" rowspan="4" align="left" valign="top">
