...
if($q=='salaryscale'){
	$sql = "SELECT Description FROM CD_TG_SalaryScale where ServiceID='$iCID'";
	$stmt = $db->runMsSqlQuery($sql);
	$row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
	if(count($row)>0){
	echo $Description=trim($row['Description']);
	}else{
		echo "Not Available. Please Contact National Cordinator.";	
	}
	
	echo "<input name=\"SalaryScale\" type=\"hidden\" class=\"input2\" id=\"SalaryScale\" value=\"$Description\"/>";
}
