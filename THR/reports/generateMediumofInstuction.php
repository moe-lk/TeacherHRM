...
$cmbMedium = $_REQUEST["cmbMedium"];
$cmbSubject = $_REQUEST["cmbSubject"];
$cmbGender = $_REQUEST["cmbGender"];
...
FROM
CD_Medium
WHERE CD_Medium.Code = '$cmbMedium'";
    $stmt6 = $db->runMsSqlQuery($sql6);
...
FROM
CD_Gender
WHERE CD_Gender.GenderCode = '$cmbGender'";
    $stmt6 = $db->runMsSqlQuery($sql6);

    while ($row6 = sqlsrv_fetch_array($stmt6, SQLSRV_FETCH_ASSOC)) {
        $GenderName = $row6["Gender Name"];
    }
} else {
    $GenderName = "All";
}
...
School Status : " . $schoolStatus . "</br>School : ". $schoolName ."</br>Medium :  ". $Medium ."</br>Subject :  ". $subjectName ."</br>Gender :  ". $GenderName ."</div>";
...
join CD_Gender on TeacherMast.GenderCode = CD_Gender.GenderCode
join CD_Subject on TeacherSubject.SubjectCode = CD_Subject.SubCode
join CD_Medium on TeacherSubject.MediumCode = CD_Medium.Code
...
if ($cmbMedium != "All") {
    $sql .= " AND TeacherSubject.MediumCode = '$cmbMedium'";
}
if ($cmbSubject != "All") {
    $sql .= " AND CD_Subject.SubCode = '$cmbSubject'";
}
if ($cmbGender != "All") {
    $sql .= " AND CD_Gender.GenderCode = '$cmbGender'";
}
if ($cmbSchoolStatus != "") {
