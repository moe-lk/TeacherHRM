...
select Distinct NIC
from TeacherSubject
join CD_Subject on TeacherSubject.SubjectCode = CD_Subject.SubCode
join CD_SubjectTypes on TeacherSubject.SubjectType = CD_SubjectTypes.SubType
where  CD_Subject.SubCode = '$cmbSubjectID' AND CD_SubjectTypes.SubType = 'APP')";

$sql .= "SELECT Distinct TeacherMast.NIC, TeacherMast.SurnameWithInitials, CD_CensesNo.InstitutionName
from TeacherMast
...
join TeacherSubject on TeacherMast.NIC = TeacherSubject.NIC
join CD_Subject on TeacherSubject.SubjectCode = CD_Subject.SubCode
join CD_SubjectTypes on TeacherSubject.SubjectType = CD_SubjectTypes.SubType
...
/*if ($cmbPositionID != "All") {
  $sql .= " AND StaffServiceHistory.PositionCode = '$cmbPositionID'";
}*/
if ($cmbSubjectID != "All") {
...
$sql .= " AND CD_SubjectTypes.SubType = 'TCH'";
$sql .= " AND TeacherMast.NIC not in (select NIC from cte)";
$sql.= " $sort $limit";
...
$sqlCount .= "SELECT Distinct TeacherMast.NIC, TeacherMast.SurnameWithInitials, CD_CensesNo.InstitutionName
from TeacherMast
join StaffServiceHistory on TeacherMast.CurServiceRef = StaffServiceHistory.ID
join CD_CensesNo on StaffServiceHistory.InstCode = CD_CensesNo.CenCode
join TeacherSubject on TeacherMast.NIC = TeacherSubject.NIC
...
/*if ($cmbPositionID != "All") {
    $sqlCount .= " AND StaffServiceHistory.PositionCode = '$cmbPositionID'";
}*/
if ($cmbSubjectID != "All") {
...
$sqlCount .= " AND CD_SubjectTypes.SubType = 'TCH'";
$sqlCount .= " AND TeacherMast.NIC not in (select NIC from cte)";
