...
$cmbFunctionalGrade = $_REQUEST["cmbFunctionalGrade"];
...
CD_SecGrades
WHERE
CD_SecGrades.GradeCode = '$cmbFunctionalGrade'";
    $stmt6 = $db->runMsSqlQuery($sql6);

    while ($row6 = sqlsrv_fetch_array($stmt6, SQLSRV_FETCH_ASSOC)) {
        $GradeName = $row6["GradeName"];
    }
} else {
    $GradeName = "All";
}
...
School Status : " . $schoolStatus . "</br>School : ". $schoolName ."</br>Functional Grade :  ". $GradeName ."</div>";
...
join CD_SecGrades on StaffServiceHistory.SecGRCode = CD_SecGrades.GradeCode
...
if ($cmbFunctionalGrade != "All") {
    $sql .= " AND CD_SecGrades.GradeCode = '$cmbFunctionalGrade'";
}
if ($cmbSchoolStatus != "") {
