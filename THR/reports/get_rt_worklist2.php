...
WHERE
StaffServiceHistory.ServiceRecTypeCode IN('NA01','TR02','DA09')
...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    $xml .= "<cell><![CDATA[".utf8_encode($row['ZoneName'])."]]></cell>";
    ...
    if(is_null($Todate)){
        if($svDateID==""){
            ...
            $xml .= "<cell><![CDATA[". $svDateID ."]]></cell>"; // Take the input date from input box
        }
    }else{
    ...
    if(is_null($Todate)){
        if($svDateID==""){
            $xml .= "<cell><![CDATA[".$NDuration->format('%y')."]]></cell>";
            $xml .= "<cell><![CDATA[".$NDuration->format('%m')."]]></cell>";
        }else{
            $xml .= "<cell><![CDATA[".$IDuration->format('%y')."]]></cell>";
            $xml .= "<cell><![CDATA[".$IDuration->format('%m')."]]></cell>";
        }
    }
    else{
        $xml .= "<cell><![CDATA[".$Duration." ]]></cell>";
        $xml .= "<cell><![CDATA[".$Duration2." ]]></cell>";
    }
