...
$cmbCategory = $_REQUEST["cmbCategory"];
...
CD_Service
WHERE
CD_Service.ServCode = '$cmbCategory'";
    $stmt6 = $db->runMsSqlQuery($sql6);

    while ($row6 = sqlsrv_fetch_array($stmt6, SQLSRV_FETCH_ASSOC)) {
        $ServiceName = $row6["ServiceName"];
    }
} else {
    $ServiceName = "All";
}
...
School Status : " . $schoolStatus . "</br>School : ". $schoolName ."</br>Service Category :  ". $ServiceName ."</br>Position :  ". $PositionName ."</div>";
...
join CD_Service on StaffServiceHistory.ServiceTypeCode = CD_Service.ServCode
...
JOIN CD_CensesCategory ON CD_CensesNo.SchoolType = CD_CensesCategory.ID
WHERE
...
if ($cmbCategory != "All") {
    $sql .= " AND CD_Service.ServCode = '$cmbCategory'";
}
if ($cmbPosition != "All") {
    $sql .= " AND CD_Positions.Code = '$cmbPosition'";
}
if ($cmbSchoolStatus != "") {
