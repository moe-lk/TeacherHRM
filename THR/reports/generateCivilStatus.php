...
$cmbCivilStatus = $_REQUEST["cmbCivilStatus"];
...
CD_CivilStatus
WHERE
CD_CivilStatus.Code = '$cmbCivilStatus'";
    $stmt6 = $db->runMsSqlQuery($sql6);

    while ($row6 = sqlsrv_fetch_array($stmt6, SQLSRV_FETCH_ASSOC)) {
        $CivilStatusName = $row6["CivilStatusName"];
    }
} else {
    $CivilStatusName = "All";
}
...
School Status : " . $schoolStatus . "</br>School : ". $schoolName ."</br>Civil Status :  ". $CivilStatusName ."</div>";
...
join CD_CivilStatus on TeacherMast.CivilStatusCode = CD_CivilStatus.Code
...
if ($cmbCivilStatus != "All") {
    $sql .= " AND CD_CivilStatus.Code = '$cmbCivilStatus'";
}
if ($cmbSchoolStatus != "") {
