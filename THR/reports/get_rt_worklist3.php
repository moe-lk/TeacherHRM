...
$sql .= ")"; // Sql query to fetch details of teachers with Service reord type of NA01 and DA05
$sql .= " $sort $limit"; //Sort according to above mentioned limits
...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    $NDuration = date_diff(date_create(date("M j, Y")) ,$Fromdate);
    ...
    $xml .= "<cell><![CDATA[".utf8_encode($row['ServiceName'])."]]></cell>";
    $xml .= "<cell><![CDATA[".utf8_encode($row['InstitutionName'])."]]></cell>";
    $xml .= "<cell><![CDATA[".($Fromdate)->format('Y/m/d')."]]></cell>";
    if(is_null($Todate)){
        if($svDateID==""){
            $xml .= "<cell><![CDATA[". date("Y/m/d") ."]]></cell>";
        }else{
            $xml .= "<cell><![CDATA[". $svDateID ."]]></cell>";
        }
    }else{
        $xml .= "<cell><![CDATA[".($Todate)->format('Y/m/d')."]]></cell>";
    }
    if(is_null($Todate)){
        if($svDateID==""){
            $xml .= "<cell><![CDATA[".$NDuration->format('%y years')."]]></cell>";
        }else{
            $xml .= "<cell><![CDATA[".$IDuration->format('%y years')."]]></cell>";
        }
    }
    else{
        $xml .= "<cell><![CDATA[".$Duration." years]]></cell>";
    }
