...
$search = $_POST['search'];
if ($search == "Y") {
    $cmbMediumID = trim($_POST['cmbMedium']);
    $cmbSubjectID = trim($_POST['cmbSubject']);
    $cmbGenderID = trim($_POST['cmbGender']);
}
//exit();
?>
        ...
        <form id="frmReport" name="frmReport" action="" method="post" enctype="multipart/form-data" >
            <div class="productsItemBoxText">
                ...
                <div style="height: 51px;">
                    ...
                    <div style="float: left;width: 279px; height:40px"><label for="username" class="labelTxt" style="margin-left:22px;"><strong>Medium of Instruction & Gender</strong></label></div>
                ...
                <div class="row">
                    <div class="column" >
                        <div style="width: 15%;float: left;margin-top: 5px; margin-left: 22px;">Medium :</div>
                        <div  style="margin-left:22px;">
                            <select class="selectRpt" style="width:260px;" id="cmbMedium" name="cmbMedium" >
                                ...
                                while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                    ...
                                    if ($cmbMediumID == trim($Code)) {
                                        echo '<option selected="selected" value=' . $Code . '>' . $Medium . '</option>';
                                    } else {
                                        echo '<option value=' . $Code . '>' . $Medium . '</option>';
                                    }
                                }
                                ?>
                            </select>
                    ...
                    <div class="column2">
                        <div style="width: 15%; margin-top: 2px; float: left;">Subject :</div>
                        <div >

                        <select class="selectRpt" style="width:260px; float: left;" id="cmbSubject" name="cmbSubject" >
                            <option value="All">All</option>
                            <?php
                            $sql = "SELECT
                            CD_Subject.SubCode,
                ...
                <div class = "row2">
                  <div class="column" >
                      <div style="width: 15%;float: left;margin-top: 5px; margin-left: 22px;">Gender :</div>
                      <div  style="margin-left:22px;">
                          <select class="selectRpt" style="width:260px;" id="cmbGender" name="cmbGender" >
                              <option value="All">All</option>
                              <?php
                              $sql = "SELECT
                              CD_Gender.GenderCode,
                              CD_Gender.[Gender Name]
                              FROM
                              CD_Gender";
                              $stmt = $db->runMsSqlQuery($sql);
                              while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                  ...
                                  if ($cmbGenderID == trim($GenderCode)) {
                                      echo '<option selected="selected" value=' . $GenderCode . '>' . $GenderName . '</option>';
                                  } else {
                                      echo '<option value=' . $GenderCode . '>' . $GenderName . '</option>';
                                  }
                              }
                              ?>
                  ...
                  <input style="margin-top:0px !important;background-color: #92495C;font-size: 12px;
                         color: #fff;padding: 4px; margin-left: 0px;
...
<script type="text/javascript">
    ...
    function submitForm() {
        if (!$("input[name='rExportXLS']").is(':checked')) {
            ...
            document.getElementById('frmReport').action = 'generateMediumofInstuction.php';
