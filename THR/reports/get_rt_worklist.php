...
$rtDateID2 = $_POST['rtDateID2'];




if (!$sortname) $sortname = 'TeacherMast.NIC';
...
if ($cmbRetirementID != "All"){
    $sql .= " AND DATEDIFF(,DOB,'$rtDate') >='60'";
  }
*/
if ( ! empty($rtDateID2)){
    // $sql .= " AND DATEDIFF(hh,DOB,'$rtDateID2') >='525960'";
    $sql .= " AND DATEADD(year, -60, '$rtDateID2') >= DOB";
}
if ( ! empty($rtDateID)){
    //   $sql .= " AND DATEDIFF(hh,DOB,'$rtDateID') <='525960'";
    $sql .= " AND DATEADD(year, -60, '$rtDateID') <= DOB";
}
if ($cmbSchoolStatus != "") {
...
if ( ! empty($rtDateID2)){
    // $sql .= " AND DATEDIFF(hh,DOB,'$rtDateID2') >='525960'";
    $sqlCount .= " AND DATEADD(year, -60, '$rtDateID2') >= DOB";
}
if ( ! empty($rtDateID)){
    //   $sql .= " AND DATEDIFF(hh,DOB,'$rtDateID') <='525960'";
    $sqlCount .= " AND DATEADD(year, -60, '$rtDateID') <= DOB";
}
if ($cmbSchoolStatus != "") {
...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    $xml .= "<cell><![CDATA[".utf8_encode($DRT->format('Y/m/d'))."]]></cell>";
	$xml .= "</row>";
